...
/* 日志文件名 */
	/*TLS*/ static char	g_log_pathfilename[MAXLEN_FILENAME + 1] = LOG_FILE_NAME;
	/*TLS*/ static int		g_log_level = LOGLEVEL_INFO;
	static int isUseStdOut = 0;
	const char log_level_itoa[][6] = { "DEBUG" , "INFO" , "WARN" , "ERROR" , "FATAL", "NOLOG" };

	void SwitchStdOut(int a) {
		if (a) {
			isUseStdOut = 1;
		}
		else {
	...
	/* 设置日志文件名 */
	void SetLogFile(const char *format, ...)
	{
		va_list		valist;
	...
	void SetLogFileV(const char *format, va_list valist)
	{
		VSNPRINTF(g_log_pathfilename, sizeof(g_log_pathfilename) - 1, format, valist);
	...
	static int WriteLogBase(int log_level,const char *c_filename, long c_fileline,const char *format, va_list valist)
	{
		...
		/* 输出行日志 */
		if (g_log_pathfilename[0] == '\0' || isUseStdOut == 1)
		{
		  
#ifdef LOG_USE_COLOR
  if(log_level == LOGLEVEL_WARN) WRITE(1,  "\33[34m", 5);
  else if(log_level == LOGLEVEL_ERROR
        || log_level == LOGLEVEL_FATAL) WRITE(1,  "\33[01;31m", 8);
#endif
			WRITE(1, log_buffer, log_buflen);
#ifdef LOG_USE_COLOR
		 if(log_level == LOGLEVEL_WARN
		  || log_level == LOGLEVEL_ERROR
		  || log_level == LOGLEVEL_FATAL) WRITE(1,  "\33[0m", 6);
#endif
		}
		//else
		{
   ...
   nret = STAT( g_log_pathfilename , &file_change_stat ) ;
   if(nret == -1) return -1;
   if(file_change_stat.st_size >=  MAX_FILE_LOG_SIZE ){
	...
	int WriteLog(int log_level,const char *c_filename, long c_fileline,const char *format, ...)
	{
		va_list		valist;
	...
	int FatalLog(const char *c_filename, long c_fileline,const char *format, ...)
	{
		va_list		valist;
	...
	int ErrorLog(const char *c_filename, long c_fileline,const char *format, ...)
	{
		va_list		valist;
	...
	int WarnLog(const char *c_filename, long c_fileline,const char *format, ...)
	{
		va_list		valist;
	...
	int InfoLog(const char *c_filename, long c_fileline,const char *format, ...)
	{
		va_list		valist;
	...
	int DebugLog(const char *c_filename, long c_fileline,const char *format, ...)
	{
		va_list		valist;
	...
	static int WriteHexLogBase(int log_level,const char *c_filename, long c_fileline,const char *buf, long buflen,const char *format, va_list valist)
	{
	...
	int WriteHexLog(int log_level,const char *c_filename, long c_fileline,const char *buf, long buflen,const char *format, ...)
	{
		va_list		valist;
	...
	int FatalHexLog(const char *c_filename, long c_fileline,const char *buf, long buflen,const char *format, ...)
	{
		va_list		valist;
	...
	int ErrorHexLog(const char *c_filename, long c_fileline,const char *buf, long buflen,const char *format, ...)
	{
		va_list		valist;
	...
	int WarnHexLog(const char *c_filename, long c_fileline,const char *buf, long buflen,const char *format, ...)
	{
		va_list		valist;
	...
	int InfoHexLog(const char *c_filename, long c_fileline,const char *buf, long buflen,const char *format, ...)
	{
		va_list		valist;
	...
	int DebugHexLog(const char *c_filename, long c_fileline,const char *buf, long buflen,const char *format, ...)
	{
		va_list		valist;
