...
namespace Key4WiFi
{
    ...
    public class WiFiMasterKey
    {
        ...
        public WiFiMasterKey(string Dhid)
        {
            _dhid = Dhid;
        }

        public static async Task<return_dhid> GETdhid()
        {
            HttpWebRequest webrequest = (HttpWebRequest)WebRequest.Create("http://sso.51y5.net/sso/fa.sec");
            webrequest.ContentType = "application/x-www-form-urlencoded";
            webrequest.Method = "POST";

            EasClientDeviceInformation ecdi = new EasClientDeviceInformation();
            webrequest.Headers["User-Agent"] = "Dalvik/1.6.0 (Linux; U; Android 4.4.2; NoxW Build/KOT49H)";
            webrequest.Headers["Host"] = "sso.51y5.net";
            ...
            WebResponse responce = await webrequest.GetResponseAsync();
        ...
        public async Task<return_pwd> ScanWiFi(IList<WiFi> wifis)
        {
            ...
            {
                sb += "{";
                sb += $"\"bssid\":\"{w.BSSID}\",\"ssid\":\"{w.SSID}\",\"securityLevel\":\"{w.SecurityLevel}\",\"rssi\":\"{w.WiFiAN.NetworkRssiInDecibelMilliwatts.ToString()}\"";
                sb += "},";
            }
            sb = sb.Remove(sb.Length - 1);
            sb += "]";
            return await ScanWiFi(sb);
        }

        public async Task<string> GetPassword(string ssid, string bssid)
        {
            Dictionary<string, string> data = new Dictionary<string, string>();
    ...
    namespace Tools
    {
        ...
        public class WiFi:INotifyPropertyChanged
        {
            ...
            public string Password { get; set; }
            public NetworkSecuritySettings SecuritySettings { get; set; }

            public string SecurityLevel { get; set; }

            private WiFiStatu statu;
            public WiFiStatu Statu
            {
                get
                {
                    return statu;
                ...
                {
                    statu = value;
                    NotifyPropertyChange("Statu");
                }
            }

            WiFiAvailableNetwork wifian;

            public WiFiAvailableNetwork WiFiAN
            {
                get
                {
                    return wifian;
                ...
                set
                {
                    ...
                    SecuritySettings = value.SecuritySettings;
                    switch(value.SecuritySettings.NetworkAuthenticationType)
                    {
                        case NetworkAuthenticationType.None:SecurityLevel = "0";break;
                        case NetworkAuthenticationType.Rsna:
                        case NetworkAuthenticationType.RsnaPsk:SecurityLevel = "2";break;
                        default:SecurityLevel = "1";break;
                    }
                }
            }
