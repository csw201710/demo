...
/*
	...
	./xxx -slave 17x.16.226.xxx  10001 17x.16.226.xxx  10002
*/
      ...
      */
      ...
      if(connect(s[1],(struct sockaddr*)&sa[1],sizeof(struct sockaddr))==0)
      {
        ...
        if(lcx_log)fprintf(lcx_log,"\n[+]  Connect %s Successed,Transfering...\n",out2),fflush(lcx_log);
				...
				#else
				  pthread_t tt;
				  pthread_create(&tt,NULL,in_data_tran,s);
				  void* __thread_return;
				  pthread_join(tt,&__thread_return);
				#endif
...
int lcx_listen(unsigned short port1,unsigned short port2)
{
  ...
  while(1)
  {
    ...
    fprintf(stdout,"\n[+]  return Waiting Connect On Port %u\n",p[i]);fflush(stdout);
    if(t[i]!=-1)
    {
...
/*
	nc 17x.16.226.xxx 10001
	nc -l -p 10002
	
./xx -tran   10001 17x.16.226.xxx  10002
*/
...
int main_func(int argc,char**argv)
{
  ...
  switch (i)
  {
    ...
    case 1:
      ...
      {
        port1 = getport(argv[n]);//获取端口1
        port2 = getport(argv[++n]);//获取端口2
      }
      break;
    case 2:
      if (argc<6)
      ...
      {
        addr1 = argv[n];// 远程地址1
        port1 = getport(argv[++n]);//远程端口1
        addr2 = argv[++n]; // 远程地址2
        port2 = getport(argv[++n]); ////远程端口2
      }
      break;
    case 3:
      if (argc<5)
      ...
      {
        port1 = getport(argv[n]);//本地端口
        addr2 = argv[++n];//远程地址
        port2 = getport(argv[++n]);//远程端口
      }
      break;
    default:
      {
  ...
  if (textpath)
  {
    setfile(&lcx_text,textpath);//打开文件
    if(lcx_text==NULL)return 0;
  }

  switch(i)
  {
    case 1:lcx_listen(port1,port2);break;//分别连接两个不同的端口转发消息
    case 2:lcx_slave(addr1,port1,addr2,port2);break;//连接远程机器端口,对数据进行转发
    case 3:lcx_tran(port1,addr2,(uint16_t)port2);break; //本地端口数据,转发到远程
    default:break;
  }
  return 0;
}
