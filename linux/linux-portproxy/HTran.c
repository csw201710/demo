 ...
 *       :
 * Notice: Thx to bkbll (bkbll#cnhonker.com)
 *       :
 * Date  : 2003-10-20
 *       :
...
int main(int argc, char* argv[])
{
    ...
    if(argc > 2)
    {
        if(strcasecmp(argv[1], "-listen") == 0 && argc >= 4)
        {
            ...
            if(strcasecmp(argv[1], "-tran") == 0 && argc >= 5)
            {
                iConnectPort = atoi(argv[2]);
                ...
                if(strcasecmp(argv[1], "-slave") == 0 && argc >= 6)
                {
...
void ver()
{
    ...
    printf("======================== Ported to Linux/Unix by 三只小潴 ======================\n\n\n");
}
...
void conn2conn(char *host1,int port1,char *host2,int port2)
{
    ...
    while(1)
    {
        ...
        while(1)
        {
            ...
            {
                if (errno == EINTR) continue; //replace WSAEINTR to EINTR
                break;
            }
